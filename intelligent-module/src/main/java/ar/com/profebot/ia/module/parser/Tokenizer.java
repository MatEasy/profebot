...
public class Tokenizer
{
    ...
    private LinkedList<sun.tools.jstat.Token> tokens;
    private static Tokenizer expressionTokenizer;

    public Tokenizer() {
        this.tokenInfos = new LinkedList<TokenInfo>();
        this.tokens = new LinkedList<sun.tools.jstat.Token>();
    }
        ...
        while (!s.equals("")) {
            ...
            for (final TokenInfo info : this.tokenInfos) {
                ...
                if (m.find()) {
                    ...
                    this.tokens.add(new sun.tools.jstat.Token(info.token, tok, totalLength - remaining));
                    break;
                }
            }
            if (!match) {
